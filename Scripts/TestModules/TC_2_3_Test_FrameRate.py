...
class TC_2_3_Class(object):
    ...
    def run_TC_2_3(self, elemetnAttributeDictionary):
        ...
        print("Test case " + tcNumber + ": Test Different Framerate: ")
        generateTestResultFile.testResultObject.writeInTestFile("\n")
        generateTestResultFile.testResultObject.writeInTestFile(",,Test and verify different FrameRate videos:\n")
        ...
        if fp.produceGolden == 1:
            ...
            numberOfVideos = len(yuvdistortedVideosList)

            for INDEX1 in range(numberOfVideos):
                if INDEX1<5:
                    yuvvideoCommandList = [fp.paths.splTestApplication, os.path.join(fp.paths.testDataDir,"2-FrameRate" , yuvdistortedVideosList[INDEX1]), os.path.join(fp.paths.testDataDir,"2-FrameRate",yuvreferenceVideosList[0] )]
                else:
                    yuvvideoCommandList = [fp.paths.splTestApplication, os.path.join(fp.paths.testDataDir,"2-FrameRate" , yuvdistortedVideosList[INDEX1]), os.path.join(fp.paths.testDataDir,"2-FrameRate",yuvreferenceVideosList[1] )]                    
        ...
        for INDEX in range(numberOfVideos):
            ...
            if INDEX < 5:
                videoCommandList = [fp.paths.sqmLibApplication, "-r", os.path.join(fp.paths.testDataDir,"2-FrameRate" , referenceVideosList[0]), "-d", os.path.join(fp.paths.testDataDir,"2-FrameRate", distortedVideosList[INDEX])]
            else:
                videoCommandList = [fp.paths.sqmLibApplication, "-r", os.path.join(fp.paths.testDataDir,"2-FrameRate" , referenceVideosList[1]), "-d", os.path.join(fp.paths.testDataDir,"2-FrameRate", distortedVideosList[INDEX])]
...
TC_2_3_ClassObject = TC_2_3_Class()
