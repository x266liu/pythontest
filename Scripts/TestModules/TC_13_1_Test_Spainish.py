...
class TC_13_1_Class(object):
    ...
    def run_TC_13_1(self, elemetnAttributeDictionary):
        ...
        generateTestResultFile.testResultObject.writeInTestFile(",,Test and verify Spanish OS:\n")
        ...
        for INDEX in range(numberOfVideos):
            ...
            if(INDEX+1<startindex):
                continue ;
            #print(" cmdlist[INDEX] = " + cmdlist[INDEX])
            if(INDEX <4):
                ...
                videoCommandList = [fp.paths.sqmLibApplication, "-r", os.path.join(fp.paths.testDataDir,folder ,referenceVideosList[1]), "-d", os.path.join(fp.paths.testDataDir,folder,distortedVideosList[1])]
            ...
            if(INDEX <4):
                ...
                lfPath = os.path.join(fp.paths.testRunLogFolder, reportFileName + str(INDEX + 1) + '_' + str(distortedVideosList[1]) + ".log")
                rfPath = os.path.join(fp.paths.testRunLogFolder, reportFileName + str(INDEX + 1) + '_' + str(distortedVideosList[1]) + "_Report.csv")
                pipePath = os.path.join(fp.paths.testRunLogFolder, reportFileName + str(INDEX + 1) + '_' + str(distortedVideosList[1]) + ".screenlog")
            ...
            if(INDEX <4):
                ...
                getFormat = str(distortedVideosList[1])
                goldReportPath = os.path.join(fp.paths.goldFileFolder, reportFileName + str(INDEX + 1) + '_' + str(distortedVideosList[1]) + "_Report.csv")    
...
TC_13_1_ClassObject = TC_13_1_Class()
