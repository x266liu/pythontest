...
class TC_10_1_14_Class(object):
    ...
    def run_TC_10_1_14(self, elemetnAttributeDictionary):
        ...
        for INDEX in range(numberOfVideos):
            ...
            tempname = str(distortedVideosList[INDEX].split(' ',1)[0])
            
            lfPath = os.path.join(fp.paths.testRunLogFolder, reportFileName  + '_' + tempname + ".log")
            rfPath = os.path.join(fp.paths.testRunLogFolder, reportFileName  + '_' + tempname + "_Report.csv")
            pipePath = os.path.join(fp.paths.testRunLogFolder, reportFileName  + '_' + tempname + ".screenlog")
            ...
            if not ('task is finished' in open(lfPath).read()):
                ...
                generateTestResultFile.testResultObject.writeInTestFile(tcNumber + "," +expectedresultList[0]+ "," + "FAIL" + "," + getFormat[-3:] + ',' + " ".join(videoCommandList) +','+  failinfo +';'+ elineinfo+wlineinfo+ ".\n")
                generateTestResultFile.testResultObject.TotalNumberOfFailTC +=1
            elif (not fp.util.verifyReportCsvPair(testReportPath, goldReportPath)):                 
                print(str(INDEX + 1) + " FAIL - Format " + getFormat + " is NOT supported. Failing condition: " + conditions[1] + "\n")  
                generateTestResultFile.testResultObject.writeInTestFile(tcNumber +  "," +expectedresultList[0]+ "," + "FAIL" + "," + getFormat[-3:] + ',' + " ".join(videoCommandList) +','+  "   Failing condition: " + conditions[1] +wlineinfo+ ".\n")
                generateTestResultFile.testResultObject.TotalNumberOfFailTC +=1  
            # Check the perf log is within the allowed range
                ...
                generateTestResultFile.testResultObject.writeInTestFile(tcNumber + "," +expectedresultList[0]+ "," + "FAIL" + "," + getFormat[-3:] + ',' + " ".join(videoCommandList) +','+  conditions[2] +wlineinfo+ ".\n")
                generateTestResultFile.testResultObject.TotalNumberOfFailTC +=1
            else:
                passDescription = ""
                ...
                generateTestResultFile.testResultObject.writeInTestFile(tcNumber +  "," +expectedresultList[0]+ "," + "PASS" + "," + getFormat[-3:] + ',' + " ".join(videoCommandList) +','+  passDescription + " " +wlineinfo+ "\n")
...
TC_10_1_14_ClassObject = TC_10_1_14_Class()
